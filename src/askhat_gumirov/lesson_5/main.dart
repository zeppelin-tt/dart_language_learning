...
void main() {
  ...
  final int firstLarger100 = increasedElements.firstWhere((element) => element > 100);
  print('Первый элемент больше 100 $firstLarger100');
  final indexOfFirstLarger100 =
      increasedElements.indexWhere((element) => element == increasedElements.firstWhere((element) => element > 100));
  print('Индекс первого элемента больше 100 $indexOfFirstLarger100');
  final newResult = increasedElements.firstWhere(compare, orElse: () => 0);
  print(newResult == 0 ? 'Массив не содержит элементов из списка $compareList' : 'Массив содержит элемент $newResult');
  final int sumOfElements = increasedElements.fold(0, (previousValue, element) => previousValue += element);
  print('Сумма элементов массива = $sumOfElements');
  ...
  print('Мап');
  final mapOfInt = stringToIntList.asMap();

  print('Золотое сечение');
  final GoldenRationSize goldenRationSize = GoldenRationSize(5);
  print(goldenRationSize);
  final mapOfGoldenRation = mapOfInt.map((key, value) => MapEntry(key, GoldenRationSize(value)));
